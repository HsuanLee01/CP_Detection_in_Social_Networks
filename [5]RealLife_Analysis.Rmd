...
```{r}
BS_fixmw_rec <- cpt.meanvar(fixmw_rec_param[593:length(model_fixwin)], method="BinSeg")

locat_BS_rec <- BS_fixmw_rec@cpts[-length(BS_fixmw_rec@cpts)]
...
locat_PELT_rec <- PELT_fixmw_rec@cpts[-length(PELT_fixmw_rec@cpts)]
...
locat_BOCD_rec <- BOCD_fixmw_rec_param[["changepoint_lists"]][["maxCPs"]][[1]]
# remove the first value of the vector, as BOCPD function indicates the first window as CP all the time
...
winpar <- data.frame("window" = 1:(length(model_fixwin)-592),
                     ...
                     "reciprocity" = fixmw_rec_param[593:length(model_fixwin)],
...
inertia_cpd.plot <- 
  ...
  geom_vline(aes(xintercept = c(locat_BS_ine + 0.1,
             ...
             size = 0.7,
             show.legend = TRUE) +
  scale_color_manual(values = c("#ef3f37", "#262161", "#fbaf41"),
...
```{r fig.height=2.5, fig.width=5.5, warning=FALSE}
reciprocity_cpd.plot <- 
  ggplot(winpar, aes(x = window, y = reciprocity)) +
  geom_point(color = brewer.pal(n = 7, name = "Set1")[6]) +
  labs(title = "Reciprocity",
       x = "Window", 
       y = "Parameter") +
  theme_bw() +
  theme(plot.title = element_text(face = "italic", family = "serif"),
        ...
        panel.grid.minor = element_blank(),
        panel.background = element_blank()) +
  geom_vline(aes(xintercept = c(locat_BOCD_rec, 
                                rep(NA, length(winpar$window)-length(locat_BOCD_rec))), 
                 color = "BOCPD", linetype = "dashed"),
             size = 0.75,
             show.legend = TRUE) +
  geom_vline(aes(xintercept = c(locat_PELT_rec - 0.1,
                                rep(NA, length(winpar$window)-length(locat_PELT_rec))), 
                 color = "PELT", linetype = "dotted"), 
             size = 0.75,
             show.legend = TRUE) +
  geom_vline(aes(xintercept = c(locat_BS_rec + 0.1,
                                rep(NA, length(winpar$window)-length(locat_BS_rec))), 
                 color = "BS", linetype = "solid"), 
             size = 0.7,
             show.legend = TRUE) +
  scale_color_manual(values = c("#ef3f37", "#262161", "#fbaf41"),
