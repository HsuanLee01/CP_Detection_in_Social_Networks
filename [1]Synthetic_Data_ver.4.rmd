...
for (i in 1: rep){
  reh_1cp_m1[[i]] <- generate(formula = formula, param = param.1cp_m1, M = 10000, covar = covar)
  print(i)
}
```
...
1. BOCD: Exact CPs Detection Method
```{r}
BOCD_ine_1cp.agg.mean_m1 <- onlineCPD(ine_1cp.agg.mean_m1, getR = T, optionalOutputs = T, truncRlim = 10^-4)
BOCD_ine_1cp.agg.mean_m1[["changepoint_lists"]][["maxCPs"]][[1]]
plot(BOCD_ine_1cp.agg.mean_m1)

BOCD_ine_1cp.agg.max_m1 <- onlineCPD(ine_1cp.agg.max_m1, getR = T, optionalOutputs = T, truncRlim = 10^-4)
...
```{r}
# obtain data-driven window widths
ddwin_1cp_m1 <- ddwindows(edgelist = reh_1cp_m1, tie_effects = formula, mintime = 2000, 
		covar = covar)
```
...
plot(fixmw_ine_1cp_param_m1, type = "lm") # plot the movement of parameter
```
...
# plot the location of detected CPs
plot(top_ine_scp, type="l", col = "red")
abline(v=PELT_ine_scp.agg.top@cpts[], col="black")
```
