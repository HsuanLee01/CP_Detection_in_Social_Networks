...
for (i in 1: rep){
  reh_ncp[[i]] <- generate(formula = formula, param = param.ncp, M = 2000, covar = covar)
  print(i)
}
```
...
windows <- REHdynamics::createwindows(200, tau)
...
plot(fixmw_ine_ncp_param, type = "lm") # plot the movement of parameter
```
...
# plot the location of CPs
plot(fixmw_ine_ncp_param, type="l", col = "red")
...
for (i in 1: rep){
  reh_1cp_m1[[i]] <- generate(formula = formula, param = param.1cp_m1, M = 2000, covar = covar)
  print(i)
}
```
...
# fit to the REM model
model_fixwin_1cp_m1 <- REHdynamics::MWrem(windows, edgelist = reh_ncp, 
                                       stats = stat.1cp_m1,
                                       actors = covar$id, directed = TRUE, 
                                       method = "MLE", model = "tie", ncores = 2)
```
