...
**Read the Data**
```{r}
REH_1CP_diff.all <- readRDS("Synthetic_Data/REH_1CP_diff.all.RDS")
```

**Extract the Statistics Used for Fitting the REM**
```{r}
# compute the statistics
out.1cp_diff.all <- list()
for (i in 1:length(REH_1CP_diff.all)) {
  out.1cp_diff.all[[i]] <- remstats(tie_effects = formula, edgelist = REH_1CP_diff.all[[i]])
}

# extract the statistics from the list
stat.1cp_diff.all <- list()
for (i in 1:length(REH_1CP_diff.all)) {
  stat.1cp_diff.all[[i]] <- out.1cp_diff.all[[i]][["statistics"]]
}
```

**Fit the Fixed MW REM on the REH with One CP (different location)**
...
tau <- 0
for (i in 1:length(REH_1CP_diff.all)) {
  tau[i] <- max(REH_1CP_diff.all[[i]]$time)
}

# specify the length of the window
windows_1cp_diff.all <- list()
for (i in 1:length(REH_1CP_diff.all)) {
  windows_1cp_diff.all[[i]] <- REHdynamics::createwindows(2000, tau[i], overlap = 2/3)
}

# fit to the REM model
model_fixwin_1cp_diff.all <- list()
for (i in 1:length(REH_1CP_diff.all)) {
  model_fixwin_1cp_diff.all[[i]] <- REHdynamics::MWrem(windows_1cp_diff.all[[i]], edgelist = REH_1CP_diff.all[[i]], 
                                       stats = stat.1cp_diff.all[[i]],
                                       actors = covar$id, directed = TRUE, 
                                       method = "MLE", model = "tie", ncores = 2)
}
```
...
```{r}
# extract the parameters
fixmw_send_1cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_1cp_diff.all))
for (i in 1:length(REH_1CP_diff.all)) {
  for (win in 1:length(model_fixwin_1cp_diff.all[[i]])) {
    fixmw_send_1cp_param_diff.all[[i]][win] <- 
      model_fixwin_1cp_diff.all[[i]][[win]][["coefficients"]][["send.z"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_diff_1cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_1cp_diff.all))
for (i in 1:length(REH_1CP_diff.all)) {
  for (win in 1:length(model_fixwin_1cp_diff.all[[i]])) {
    fixmw_diff_1cp_param_diff.all[[i]][win] <- 
      model_fixwin_1cp_diff.all[[i]][[win]][["coefficients"]][["difference.z"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_ine_1cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_1cp_diff.all))
for (i in 1:length(REH_1CP_diff.all)) {
  for (win in 1:length(model_fixwin_1cp_diff.all[[i]])) {
    fixmw_ine_1cp_param_diff.all[[i]][win] <- 
      model_fixwin_1cp_diff.all[[i]][[win]][["coefficients"]][["inertia"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_out_1cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_1cp_diff.all))
for (i in 1:length(REH_1CP_diff.all)) {
  for (win in 1:length(model_fixwin_1cp_diff.all[[i]])) {
    fixmw_out_1cp_param_diff.all[[i]][win] <- 
      model_fixwin_1cp_diff.all[[i]][[win]][["coefficients"]][["outdegreeSender"]]
  }
}
```
...
```{r}
saveRDS(object = fixmw_send_1cp_param_diff.all, file = "fixmw_send_1cp_param_diff.all.RDS")
```

```{r}
saveRDS(object = fixmw_diff_1cp_param_diff.all, file = "fixmw_diff_1cp_param_diff.all.RDS")
```

```{r}
saveRDS(object = fixmw_ine_1cp_param_diff.all, file = "fixmw_ine_1cp_param_diff.all.RDS")
```

```{r}
saveRDS(object = fixmw_out_1cp_param_diff.all, file = "fixmw_out_1cp_param_diff.all.RDS")
```
...
**Read the Data**
```{r}
REH_2CP_diff.all <- readRDS("Synthetic_Data/REH_2CP_diff.all.RDS")
```

**Extract the Statistics Used for Fitting the REM**
```{r}
# compute the statistics
out.2cp_diff.all <- list()
for (i in 1:length(REH_2CP_diff.all)) {
  out.2cp_diff.all[[i]] <- remstats(tie_effects = formula, edgelist = REH_2CP_diff.all[[i]])
}

# extract the statistics from the list
stat.2cp_diff.all <- list()
for (i in 1:length(REH_2CP_diff.all)) {
  stat.2cp_diff.all[[i]] <- out.2cp_diff.all[[i]][["statistics"]]
}
```
...
tau <- 0
for (i in 1:length(REH_2CP_diff.all)) {
  tau[i] <- max(REH_2CP_diff.all[[i]]$time)
}

# specify the length of the window
windows_2cp_diff.all <- list()
for (i in 1:length(REH_2CP_diff.all)) {
  windows_2cp_diff.all[[i]] <- REHdynamics::createwindows(2000, tau[i], overlap = 2/3)
}

# fit to the REM model
model_fixwin_2cp_diff.all <- list()
for (i in 1:length(REH_2CP_diff.all)) {
  model_fixwin_2cp_diff.all[[i]] <- REHdynamics::MWrem(windows_2cp_diff.all[[i]], edgelist = REH_2CP_diff.all[[i]], 
                                       stats = stat.2cp_diff.all[[i]],
                                       actors = covar$id, directed = TRUE, 
                                       method = "MLE", model = "tie", ncores = 2)
}
```
...
```{r}
# extract the parameters
fixmw_send_2cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_2cp_diff.all))
for (i in 1:length(REH_2CP_diff.all)) {
  for (win in 1:length(model_fixwin_2cp_diff.all[[i]])) {
    fixmw_send_2cp_param_diff.all[[i]][win] <- 
      model_fixwin_2cp_diff.all[[i]][[win]][["coefficients"]][["send.z"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_diff_2cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_2cp_diff.all))
for (i in 1:length(REH_2CP_diff.all)) {
  for (win in 1:length(model_fixwin_2cp_diff.all[[i]])) {
    fixmw_diff_2cp_param_diff.all[[i]][win] <- 
      model_fixwin_2cp_diff.all[[i]][[win]][["coefficients"]][["difference.z"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_ine_2cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_2cp_diff.all))
for (i in 1:length(REH_2CP_diff.all)) {
  for (win in 1:length(model_fixwin_2cp_diff.all[[i]])) {
    fixmw_ine_2cp_param_diff.all[[i]][win] <- 
      model_fixwin_2cp_diff.all[[i]][[win]][["coefficients"]][["inertia"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_out_2cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_2cp_diff.all))
for (i in 1:length(REH_2CP_diff.all)) {
  for (win in 1:length(model_fixwin_2cp_diff.all[[i]])) {
    fixmw_out_2cp_param_diff.all[[i]][win] <- 
      model_fixwin_2cp_diff.all[[i]][[win]][["coefficients"]][["outdegreeSender"]]
  }
}
```
...
```{r}
saveRDS(object = fixmw_send_2cp_param_diff.all, file = "fixmw_send_2cp_param_diff.all.RDS")
```

```{r}
saveRDS(object = fixmw_diff_2cp_param_diff.all, file = "fixmw_diff_2cp_param_diff.all.RDS")
```

```{r}
saveRDS(object = fixmw_ine_2cp_param_diff.all, file = "fixmw_ine_2cp_param_diff.all.RDS")
```

```{r}
saveRDS(object = fixmw_out_2cp_param_diff.all, file = "fixmw_out_2cp_param_diff.all.RDS")
```
...
**Read the Data**
```{r}
REH_3CP <- readRDS("Synthetic_Data/REH_3CP.RDS")
```
...
# compute the statistics
out.3cp <- list()
for (i in 1:length(REH_3CP)) {
  out.3cp[[i]] <- remstats(tie_effects = formula, edgelist = REH_3CP[[i]])
}

# extract the statistics from the list
stat.3cp <- list()
for (i in 1:length(REH_3CP)) {
  stat.3cp[[i]] <- out.3cp[[i]][["statistics"]]
}
```

**Fit the Fixed MW REM on the REH with Three CPs (inertia only)**
...
tau <- 0
for (i in 1:length(REH_3CP)) {
  tau[i] <- max(REH_3CP[[i]]$time)
}

# specify the length of the window
windows_3cp <- list()
for (i in 1:length(REH_3CP)) {
  windows_3cp[[i]] <- REHdynamics::createwindows(1000, tau[i], overlap = 1/2)
}

# fit to the REM model
model_fixwin_3cp <- list()
for (i in 1:length(REH_3CP)) {
  model_fixwin_3cp[[i]] <- REHdynamics::MWrem(windows_3cp[[i]], edgelist = REH_3CP[[i]], 
                                       stats = stat.3cp[[i]],
                                       actors = covar$id, directed = TRUE, 
                                       method = "MLE", model = "tie", ncores = 2)
}
...
```{r}
# extract the parameters
fixmw_send_3cp_param <- vector(mode='list', length = length(model_fixwin_3cp))
for (i in 1:length(REH_3CP)) {
  for (win in 1:length(model_fixwin_3cp[[i]])) {
    fixmw_send_3cp_param[[i]][win] <- 
      model_fixwin_3cp[[i]][[win]][["coefficients"]][["send.z"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_diff_3cp_param <- vector(mode='list', length = length(model_fixwin_3cp))
for (i in 1:length(REH_3CP)) {
  for (win in 1:length(model_fixwin_3cp[[i]])) {
    fixmw_diff_3cp_param[[i]][win] <- 
      model_fixwin_3cp[[i]][[win]][["coefficients"]][["difference.z"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_ine_3cp_param <- vector(mode='list', length = length(model_fixwin_3cp))
for (i in 1:length(REH_3CP)) {
  for (win in 1:length(model_fixwin_3cp[[i]])) {
    fixmw_ine_3cp_param[[i]][win] <- 
      model_fixwin_3cp[[i]][[win]][["coefficients"]][["inertia"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_out_3cp_param <- vector(mode='list', length = length(model_fixwin_3cp))
for (i in 1:length(REH_3CP)) {
  for (win in 1:length(model_fixwin_3cp[[i]])) {
    fixmw_out_3cp_param[[i]][win] <- 
      model_fixwin_3cp[[i]][[win]][["coefficients"]][["outdegreeSender"]]
  }
}
```
...
**Read the Data**
```{r}
REH_3CP_same.all <- readRDS("Synthetic_Data/REH_3CP_same.all.RDS")
```

**Extract the Statistics Used for Fitting the REM**
```{r}
# compute the statistics
out.3cp_same.all <- list()
for (i in 1:length(REH_3CP_same.all)) {
  out.3cp_same.all[[i]] <- remstats(tie_effects = formula, edgelist = REH_3CP_same.all[[i]])
}

# extract the statistics from the list
stat.3cp_same.all <- list()
for (i in 1:length(REH_3CP_same.all)) {
  stat.3cp_same.all[[i]] <- out.3cp_same.all[[i]][["statistics"]]
}
```
...
tau <- 0
for (i in 1:length(REH_3CP_same.all)) {
  tau[i] <- max(REH_3CP_same.all[[i]]$time)
}

# specify the length of the window
windows_3cp_same.all <- list()
for (i in 1:length(REH_3CP_same.all)) {
  windows_3cp_same.all[[i]] <- REHdynamics::createwindows(1000, tau[i], overlap = 1/2)
}

# fit to the REM model
model_fixwin_3cp_same.all <- list()
for (i in 1:length(REH_3CP_same.all)) {
  model_fixwin_3cp_same.all[[i]] <- REHdynamics::MWrem(windows_3cp_same.all[[i]], edgelist = REH_3CP_same.all[[i]], 
                                       stats = stat.3cp_same.all[[i]],
                                       actors = covar$id, directed = TRUE, 
                                       method = "MLE", model = "tie", ncores = 2)
}
```
...
```{r}
# extract the parameters
fixmw_send_3cp_param_same.all <- vector(mode='list', length = length(model_fixwin_3cp_same.all))
for (i in 1:length(REH_3CP_same.all)) {
  for (win in 1:length(model_fixwin_3cp_same.all[[i]])) {
    fixmw_send_3cp_param_same.all[[i]][win] <- 
      model_fixwin_3cp_same.all[[i]][[win]][["coefficients"]][["send.z"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_diff_3cp_param_same.all <- vector(mode='list', length = length(model_fixwin_3cp_same.all))
for (i in 1:length(REH_3CP_same.all)) {
  for (win in 1:length(model_fixwin_3cp_same.all[[i]])) {
    fixmw_diff_3cp_param_same.all[[i]][win] <- 
      model_fixwin_3cp_same.all[[i]][[win]][["coefficients"]][["difference.z"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_ine_3cp_param_same.all <- vector(mode='list', length = length(model_fixwin_3cp_same.all))
for (i in 1:length(REH_3CP_same.all)) {
  for (win in 1:length(model_fixwin_3cp_same.all[[i]])) {
    fixmw_ine_3cp_param_same.all[[i]][win] <- 
      model_fixwin_3cp_same.all[[i]][[win]][["coefficients"]][["inertia"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_out_3cp_param_same.all <- vector(mode='list', length = length(model_fixwin_3cp_same.all))
for (i in 1:length(REH_3CP_same.all)) {
  for (win in 1:length(model_fixwin_3cp_same.all[[i]])) {
    fixmw_out_3cp_param_same.all[[i]][win] <- 
      model_fixwin_3cp_same.all[[i]][[win]][["coefficients"]][["outdegreeSender"]]
  }
}
```
...
**Read the Data**
```{r}
REH_3CP_diff.all <- readRDS("Synthetic_Data/REH_3CP_diff.all.RDS")
```

**Extract the Statistics Used for Fitting the REM**
```{r}
# compute the statistics
out.3cp_diff.all <- list()
for (i in 1:length(REH_3CP_diff.all)) {
  out.3cp_diff.all[[i]] <- remstats(tie_effects = formula, edgelist = REH_3CP_diff.all[[i]])
}

# extract the statistics from the list
stat.3cp_diff.all <- list()
for (i in 1:length(REH_3CP_diff.all)) {
  stat.3cp_diff.all[[i]] <- out.3cp_diff.all[[i]][["statistics"]]
}
```
...
tau <- 0
for (i in 1:length(REH_3CP_diff.all)) {
  tau[i] <- max(REH_3CP_diff.all[[i]]$time)
}

# specify the length of the window
windows_3cp_diff.all <- list()
for (i in 1:length(REH_3CP_diff.all)) {
  windows_3cp_diff.all[[i]] <- REHdynamics::createwindows(1000, tau[i], overlap = 1/2)
}

# fit to the REM model
model_fixwin_3cp_diff.all <- list()
for (i in 1:length(REH_3CP_diff.all)) {
  model_fixwin_3cp_diff.all[[i]] <- REHdynamics::MWrem(windows_3cp_diff.all[[i]], edgelist = REH_3CP_diff.all[[i]], 
                                       stats = stat.3cp_diff.all[[i]],
                                       actors = covar$id, directed = TRUE, 
                                       method = "MLE", model = "tie", ncores = 2)
}
```
...
```{r}
# extract the parameters
fixmw_send_3cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_3cp_diff.all))
for (i in 1:length(REH_3CP_diff.all)) {
  for (win in 1:length(model_fixwin_3cp_diff.all[[i]])) {
    fixmw_send_3cp_param_diff.all[[i]][win] <- 
      model_fixwin_3cp_diff.all[[i]][[win]][["coefficients"]][["send.z"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_diff_3cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_3cp_diff.all))
for (i in 1:length(REH_3CP_diff.all)) {
  for (win in 1:length(model_fixwin_3cp_diff.all[[i]])) {
    fixmw_diff_3cp_param_diff.all[[i]][win] <- 
      model_fixwin_3cp_diff.all[[i]][[win]][["coefficients"]][["difference.z"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_ine_3cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_3cp_diff.all))
for (i in 1:length(REH_3CP_diff.all)) {
  for (win in 1:length(model_fixwin_3cp_diff.all[[i]])) {
    fixmw_ine_3cp_param_diff.all[[i]][win] <- 
      model_fixwin_3cp_diff.all[[i]][[win]][["coefficients"]][["inertia"]]
  }
}
```
...
```{r}
# extract the parameters
fixmw_out_3cp_param_diff.all <- vector(mode='list', length = length(model_fixwin_3cp_diff.all))
for (i in 1:length(REH_3CP_diff.all)) {
  for (win in 1:length(model_fixwin_3cp_diff.all[[i]])) {
    fixmw_out_3cp_param_diff.all[[i]][win] <- 
      model_fixwin_3cp_diff.all[[i]][[win]][["coefficients"]][["outdegreeSender"]]
  }
}
```
