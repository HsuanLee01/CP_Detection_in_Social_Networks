...
Sender Effect:
```{r}
locat_BOCD_ncp_send <- readRDS("CP_Detection/locat_BOCD_ncp_send.RDS")
```

Difference Effect: 
```{r}
locat_BOCD_ncp_diff <- readRDS("CP_Detection/locat_BOCD_ncp_diff.RDS")
```

Inertia:
```{r}
locat_BOCD_ncp_ine <- readRDS("CP_Detection/locat_BOCD_ncp_ine.RDS")
```

Out Degree Sender Effect:
```{r}
locat_BOCD_ncp_out <- readRDS("CP_Detection/locat_BOCD_ncp_out.RDS")
```
...
Sender Effect:
```{r}
locat_PELT_ncp_send <- readRDS("CP_Detection/locat_PELT_ncp_send.RDS")
```

Difference Effect: 
```{r}
locat_PELT_ncp_diff <- readRDS("CP_Detection/locat_PELT_ncp_diff.RDS")
```

Inertia:
```{r}
locat_PELT_ncp_ine <- readRDS("CP_Detection/locat_PELT_ncp_ine.RDS")
```

Out Degree Sender Effect:
```{r}
locat_PELT_ncp_out <- readRDS("CP_Detection/locat_PELT_ncp_out.RDS")
```
...
Sender Effect:
```{r}
locat_BS_ncp_send <- readRDS("CP_Detection/locat_BS_ncp_send.RDS")
```

Difference Effect: 
```{r}
locat_BS_ncp_diff <- readRDS("CP_Detection/locat_BS_ncp_diff.RDS")
```

Inertia:
```{r}
locat_BS_ncp_ine <- readRDS("CP_Detection/locat_BS_ncp_ine.RDS")
```

Out Degree Sender Effect:
```{r}
locat_BS_ncp_out <- readRDS("CP_Detection/locat_BS_ncp_out.RDS")
```
...
Sender Effect:
```{r}
locat_BOCD_1cp_send <- readRDS("CP_Detection/locat_BOCD_1cp_send.RDS")
```

Difference Effect: 
```{r}
locat_BOCD_1cp_diff <- readRDS("CP_Detection/locat_BOCD_1cp_diff.RDS")
```

Inertia:
```{r}
locat_BOCD_1cp_ine <- readRDS("CP_Detection/locat_BOCD_1cp_ine.RDS")
```

Out Degree Sender Effect:
```{r}
locat_BOCD_1cp_out <- readRDS("CP_Detection/locat_BOCD_1cp_out.RDS")
```
...
Sender Effect:
```{r}
locat_PELT_1cp_send <- readRDS("CP_Detection/locat_PELT_1cp_send.RDS")
```

Difference Effect: 
```{r}
locat_PELT_1cp_diff <- readRDS("CP_Detection/locat_PELT_1cp_diff.RDS")
```

Inertia:
```{r}
locat_PELT_1cp_ine <- readRDS("CP_Detection/locat_PELT_1cp_ine.RDS")
```

Out Degree Sender Effect:
```{r}
locat_PELT_1cp_out <- readRDS("CP_Detection/locat_PELT_1cp_out.RDS")
```
...
Sender Effect:
```{r}
locat_BS_1cp_send <- readRDS("CP_Detection/locat_BS_1cp_send.RDS")
```

Difference Effect: 
```{r}
locat_BS_1cp_diff <- readRDS("CP_Detection/locat_BS_1cp_diff.RDS")
```

Inertia:
```{r}
locat_BS_1cp_ine <- readRDS("CP_Detection/locat_BS_1cp_ine.RDS")
```

Out Degree Sender Effect:
```{r}
locat_BS_1cp_out <- readRDS("CP_Detection/locat_BS_1cp_out.RDS")
```
...
Sender Effect:
```{r}
locat_BOCD_1cp_send_same.all <- readRDS("CP_Detection/locat_BOCD_1cp_send_same.all.RDS")
```

Difference Effect: 
```{r}
locat_BOCD_1cp_diff_same.all <- readRDS("CP_Detection/locat_BOCD_1cp_diff_same.all.RDS")
```

Inertia:
```{r}
locat_BOCD_1cp_ine_same.all <- readRDS("CP_Detection/locat_BOCD_1cp_ine_same.all.RDS")
```

Out Degree Sender Effect:
```{r}
locat_BOCD_1cp_out_same.all <- readRDS("CP_Detection/locat_BOCD_1cp_out_same.all.RDS")
```
...
Sender Effect:
```{r}
locat_PELT_1cp_send_same.all <- readRDS("CP_Detection/locat_PELT_1cp_send_same.all.RDS")
```

Difference Effect: 
```{r}
locat_PELT_1cp_diff_same.all <- readRDS("CP_Detection/locat_PELT_1cp_diff_same.all.RDS")
```

Inertia:
```{r}
locat_PELT_1cp_ine_same.all <- readRDS("CP_Detection/locat_PELT_1cp_ine_same.all.RDS")
```

Out Degree Sender Effect:
```{r}
locat_PELT_1cp_out_same.all <- readRDS("CP_Detection/locat_PELT_1cp_out_same.all.RDS")
```
...
Sender Effect:
```{r}
locat_BS_1cp_send_same.all <- readRDS("CP_Detection/locat_BS_1cp_send_same.all.RDS")
```

Difference Effect: 
```{r}
locat_BS_1cp_diff_same.all <- readRDS("CP_Detection/locat_BS_1cp_diff_same.all.RDS")
```

Inertia:
```{r}
locat_BS_1cp_ine_same.all <- readRDS("CP_Detection/locat_BS_1cp_ine_same.all.RDS")
```

Out Degree Sender Effect:
```{r}
locat_BS_1cp_out_same.all <- readRDS("CP_Detection/locat_BS_1cp_out_same.all.RDS")
```
...
Sender Effect:
```{r}
locat_BOCD_2cp_send <- readRDS("CP_Detection/locat_BOCD_2cp_send.RDS")
```

Difference Effect: 
```{r}
locat_BOCD_2cp_diff <- readRDS("CP_Detection/locat_BOCD_2cp_diff.RDS")
```

Inertia:
```{r}
locat_BOCD_2cp_ine <- readRDS("CP_Detection/locat_BOCD_2cp_ine.RDS")
```

Out Degree Sender Effect:
```{r}
locat_BOCD_2cp_out <- readRDS("CP_Detection/locat_BOCD_2cp_out.RDS")
```
...
Sender Effect:
```{r}
locat_PELT_2cp_send <- readRDS("CP_Detection/locat_PELT_2cp_send.RDS")
```

Difference Effect: 
```{r}
locat_PELT_2cp_diff <- readRDS("CP_Detection/locat_PELT_2cp_diff.RDS")
```

Inertia:
```{r}
locat_PELT_2cp_ine <- readRDS("CP_Detection/locat_PELT_2cp_ine.RDS")
```

Out Degree Sender Effect:
```{r}
locat_PELT_2cp_out <- readRDS("CP_Detection/locat_PELT_2cp_out.RDS")
```
...
Sender Effect:
```{r}
locat_BS_2cp_send <- readRDS("CP_Detection/locat_BS_2cp_send.RDS")
```

Difference Effect: 
```{r}
locat_BS_2cp_diff <- readRDS("CP_Detection/locat_BS_2cp_diff.RDS")
```

Inertia:
```{r}
locat_BS_2cp_ine <- readRDS("CP_Detection/locat_BS_2cp_ine.RDS")
```

Out Degree Sender Effect:
```{r}
locat_BS_2cp_out <- readRDS("CP_Detection/locat_BS_2cp_out.RDS")
```
...
Sender Effect:
```{r}
locat_BOCD_2cp_send_same.all <- readRDS("CP_Detection/locat_BOCD_2cp_send_same.all.RDS")
```

Difference Effect: 
```{r}
locat_BOCD_2cp_diff_same.all <- readRDS("CP_Detection/locat_BOCD_2cp_diff_same.all.RDS")
```

Inertia:
```{r}
locat_BOCD_2cp_ine_same.all <- readRDS("CP_Detection/locat_BOCD_2cp_ine_same.all.RDS")
```

Out Degree Sender Effect:
```{r}
locat_BOCD_2cp_out_same.all <- readRDS("CP_Detection/locat_BOCD_2cp_out_same.all.RDS")
```
...
Sender Effect:
```{r}
locat_PELT_2cp_send_same.all <- readRDS("CP_Detection/locat_PELT_2cp_send_same.all.RDS")
```

Difference Effect: 
```{r}
locat_PELT_2cp_diff_same.all <- readRDS("CP_Detection/locat_PELT_2cp_diff_same.all.RDS")
```

Inertia:
```{r}
locat_PELT_2cp_ine_same.all <- readRDS("CP_Detection/locat_PELT_2cp_ine_same.all.RDS")
```

Out Degree Sender Effect:
```{r}
locat_PELT_2cp_out_same.all <- readRDS("CP_Detection/locat_PELT_2cp_out_same.all.RDS")
```
...
Sender Effect:
```{r}
locat_BS_2cp_send_same.all <- readRDS("CP_Detection/locat_BS_2cp_send_same.all.RDS")
```

Difference Effect: 
```{r}
locat_BS_2cp_diff_same.all <- readRDS("CP_Detection/locat_BS_2cp_diff_same.all.RDS")
```

Inertia:
```{r}
locat_BS_2cp_ine_same.all <- readRDS("CP_Detection/locat_BS_2cp_ine_same.all.RDS")
```

Out Degree Sender Effect:
```{r}
locat_BS_2cp_out_same.all <- readRDS("CP_Detection/locat_BS_2cp_out_same.all.RDS")
```
