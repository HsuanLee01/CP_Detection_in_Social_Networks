...
MSE_CPD <- function(pred_CPs, true_CPs, range){
  ...
  for (i in 1:length(pred_CPs)) {
    ...
    if(length(pred_CPs[[i]]) > 0){
      ...
      # Remove NAs from initial_MSE
      initial_MSE[[i]] <- na.omit(initial_MSE[[i]])
    }
    
    # extract values from initial_MSE and sum the square error of each CP
    MSE[[i]] <- sum(unlist(initial_MSE[[i]]), na.rm = T) / length(initial_MSE[[i]])
    
  }
  
  total_MSE <- mean(unlist(MSE), na.rm = T)
  
  return(list(initial_MSE, MSE, total_MSE))
}
